...
func show_next_month() -> void:
	...
	if not (last_month.month_num ==  campaign_data.calendar_data.current_shown_month_num and last_month.year_num == campaign_data.calendar_data.current_shown_year_num):
		var new_month = campaign_data.calendar_data.current_shown_month_num + 1
		var new_year = campaign_data.calendar_data.current_shown_year_num
		if new_month > 12:
			new_year = new_year + 1
			new_month = 1
		campaign_data.calendar_data.current_shown_month_num = new_month
		campaign_data.calendar_data.current_shown_year_num = new_year
		
func show_previous_month() -> void:
	var first_month = get_current_season_if_active().get_first_month()
	if not (first_month.month_num ==  campaign_data.calendar_data.current_shown_month_num and first_month.year_num == campaign_data.calendar_data.current_shown_year_num):
		var new_month = campaign_data.calendar_data.current_shown_month_num - 1
		var new_year = campaign_data.calendar_data.current_shown_year_num
		if new_month < 1:
			new_year = new_year - 1
...
func next_day() -> void:
	var next_day: CalendaricDay = campaign_data.calendar_data.current_day.next_day()
	if get_current_season_if_active().get_last_day().day.is_after(next_day):
		var new_day_event = NextDayStartedEvent.new(campaign_data.calendar_data.current_day, next_day)
		campaign_data.calendar_data.current_day = next_day
		campaign_data.calendar_data.is_after_main_activity = false
...
func activate_main_activity() -> void:
	if not campaign_data.calendar_data.is_after_main_activity:
		var current_day: CoachingDay = campaign_data.get_current_day()
...
func finish_day() -> void:
	campaign_data.calendar_data.is_after_main_activity = true
...
func select_day(_year_num: int, _month_num: int, _day_num: int) -> void:
	...
	campaign_data.calendar_data.selected_day = _new_selected_day
...
func show_calendaric_day(_selected: CalendaricDay) -> void:
	campaign_data.calendar_data.current_shown_month_num = _selected.month
	campaign_data.calendar_data.current_shown_year_num = _selected.year
